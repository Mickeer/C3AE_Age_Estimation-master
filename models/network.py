...
def h_swish(inputs):
    return inputs * tf.nn.relu6(inputs + 3) / 6.   #限制最大输出值为6的ReLU函数

def network_arg_scope(    #一些参数范围的定义
...
class Network(object):
    ...
    def inference(self, mode, inputs, scope='C3AE'):     #定义五个标准卷积和两个全连接层
        is_training = mode
        with slim.arg_scope(network_arg_scope(is_training=is_training)):
            with tf.variable_scope(scope, reuse=False):
                ...
                concat = reshape(conv5, [-1, conv5.get_shape()[1] * conv5.get_shape()[2] * conv5.get_shape()[3] * 3], 'reshape') #引入级联
...
def se_module(inputs, c_outputs, s, name):          #引入SE#压缩奖惩网络模块（Squeeze-and-Excitation module）模块
...
def reshape(inputs, shape, name):   #使用reshape进行矩阵变换
    output = tf.reshape(inputs, shape=shape, name=name)
    print(name, output.get_shape())
    return output

def avg_pool(inputs, name):                          #平均池化
...
def conv2d(inputs, c_outputs, s, name):
    output = slim.conv2d(inputs,                            #conv2d（）自带卷积功能+激励函数；  指需要做卷积的输入图像
                         num_outputs=c_outputs,             #指定卷积核的个数
                         kernel_size=[3,3],                 #指定卷积核的维度
                         stride=s,                          #为卷积时在图像每一维的步长
                         scope=name)                        #共享变量所指的variable_scope
    if PRINT_LAYER_LOG:
        print(name, output.get_shape())
    return output

def maxpool2x2(input, name):                                #2D最大池化
...
def d_p_conv(inputs, c_outputs, s, name):              #深度可分离卷积
...
def dw_conv(inputs, s, name):             #深度方向卷积
    output = slim.separable_convolution2d(inputs,
                                          num_outputs=None,
                                          stride=s,
                                          depth_multiplier=1,
...
def pw_conv(inputs, c_outputs, name):    #逐点卷积
...
def route(input_list, name):    #级联  拼接张量的函数
